...
impl SqliteRepository {
    ...
    pub(crate) async fn default() -> Result<SqliteRepository> {
        Self::connect(env::current_exe()?.join("..").join("note.db").to_str().ok_or(anyhow!("Could not evaluate path"))?.to_string()).await
    }
    async fn connect(conn_str: String) -> Result<SqliteRepository> {
        let conn = SqliteConnectOptions::from_str(&conn_str.to_owned())?.create_if_missing(true).connect().await?;
...
impl NoteRepository for SqliteRepository {
    ...
    async fn remove_note(&mut self, key: String) -> Result<u32> {
        let count: u32 = sqlx::query("delete from note where id=?;").bind(key.to_string()).execute(self.conn_mut()).await?.rows_affected() as u32;
        Ok(count)
    }

    async fn get_note(&mut self, key: String) -> Result<Note> {
        let item: Note = sqlx::query_as("select * from note where id like '?%';").bind(key.to_string()).fetch_one(self.conn_mut()).await?;
        Ok(item)
    }

    async fn list_note(&mut self) -> Result<Vec<Note>> {
        let items: Vec<Note> = sqlx::query_as("select * from note;").fetch_all(self.conn_mut()).await?;
        Ok(items)
    }

    async fn list_note_with_filter<T: Fn(&Note) -> bool + Send + Sync>(&mut self, pred: T) -> Result<Vec<Note>> {
    ...
    async fn update_note(&mut self, key: String, text: String) -> Result<()> {
        sqlx::query("update note set content=? where id like '?%';").bind(text).bind(key.to_string()).execute(self.conn_mut()).await?;
        Ok(())
    }
}
...
impl ProjectRepository for SqliteRepository {
    ...
    async fn insert_project(&mut self, entity: Project) -> Result<()> {
        sqlx::query("insert into project(id,name,ts) values(?,?,?);").bind(entity.guid().to_string()).bind(entity.name()).bind(entity.ts().to_string()).execute(self.conn_mut()).await?;
        Ok(())
    }

    async fn remove_project(&mut self, key: String) -> Result<u32> {
        let count: u32 = sqlx::query("delete from project where id like '?%';").bind(key.to_string()).execute(self.conn_mut()).await?.rows_affected() as u32;
        Ok(count)
    }

    async fn get_project(&mut self, key: String) -> Result<Project> {
        let item: Project = sqlx::query_as("select * from project where id like '?%';").bind(key.to_string()).fetch_one(self.conn_mut()).await?;
        Ok(item)
    }

    async fn list_project(&mut self) -> Result<Vec<Project>> {
        let items: Vec<Project> = sqlx::query_as("select * from project;").fetch_all(self.conn_mut()).await?;
        Ok(items)
    }

    async fn list_project_with_filter<T: Fn(&Project) -> bool + Send + Sync>(&mut self, pred: T) -> Result<Vec<Project>> {
...
fn apply_filter<T: Clone, K: Fn(&T) -> bool + Send + Sync>(pred: K, items: Vec<T>) -> Vec<T> {
    items.iter().filter_map(|it| {
        if pred(it) { Some(it.to_owned()) } else { None }
    }).collect_vec()
}